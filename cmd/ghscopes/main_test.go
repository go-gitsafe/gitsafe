...
func TestWhatComesBackIsReported(t *testing.T) {
	answering(t, 200, "repo, workflow", `{"login":"a-user"}`)
	var out, errOut bytes.Buffer
	if code := run([]string{"-f", aToken(t), "repo"}, &out, &errOut); code != 0 {
		t.Fatalf("exit %d: %s", code, errOut.String())
	}
	got := out.String()
	for _, want := range []string{"a-user", "repo, workflow", "has:     repo", "file:"} {
...
func TestAScopeThatIsNotThereIsARefusal(t *testing.T) {
	answering(t, 200, "repo", `{"login":"a-user"}`)
	var out, errOut bytes.Buffer
...
func TestScopesAreAHierarchy(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got := missingFrom(tc.have, tc.want)
			if len(got) != len(tc.gone) {
				t.Fatalf("missingFrom(%v, %v) = %v, want %v", tc.have, tc.want, got, tc.gone)
			}
			for i := range got {
				if got[i] != tc.gone[i] {
