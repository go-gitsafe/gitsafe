// git-credential-tannevaled hands git a GitHub token without the token ever
...
func run(args []string, stdin *os.File, stdout *os.File, stderr *os.File) int {
	...
	if isTerminal(stdout) {
		fmt.Fprintln(stderr, "git-credential-tannevaled: refusing to write a credential to a terminal")
		return 1
	}
	...
	tok, err := readToken()
	if err != nil {
		fmt.Fprintf(stderr, "git-credential-tannevaled: %v\n", err)
		return 1
	}
